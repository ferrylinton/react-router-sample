...
import { countTodoes, deleteTodoById, findTodoById, findTodoes, updateTodo } from "~/.server/services/todo-service";
...
export async function action({ request }: Route.ActionArgs) {
  ...
  if (payload.action === "delete") {
    const id = payload.id;
    if (id) {
      const data = await findTodoById(id.valueOf() as string);
      if (data) {
        await deleteTodoById(id.valueOf() as string);
        return { message: `Data [task="${data.task}"] is deleted` }
      } else {
        return { message: `Data [id="${id.valueOf()}"] is not found` }
      };
    }
    ...
    if (id) {
      ...
      if (data) {
        ...
        return { message: `Data [task="${data.task}"] is updated` }
      } else {
        return { message: `Data [id="${id.valueOf()}"] is not found` }
      };
    }
...
export default function Home({ loaderData }: Route.ComponentProps) {
  ...
  const successMessage = loaderData.flashData?.message || actionData?.message;

  const errorMessage = loaderData.flashData?.errorMessage || actionData?.errorMessage;
  ...
  return <>
    <div className="w-full h-dvh flex justify-center">
      <div className="w-full max-w-xl flex flex-col gap-2 p-2">
        ...
        <Message id={uuidv4()} successMessage={successMessage} errorMessage={errorMessage} />
